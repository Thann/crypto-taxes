	...
	for (const sell of sells) {
		...
		while (buyIndex < buys.length) {
			...
			if (partialTotal + partialRemainder >= sell.cryptoAmount) {
				partialRemainder += partialTotal - sell.cryptoAmount;
				//TODO: correct?
				fiatBuys += (1 - (partialRemainder / buy.cryptoAmount)) * buy.fiatAmount
				console.log("        %", 1- (partialRemainder / buy.cryptoAmount), "  --  ", partialRemainder, ' / ',buy.cryptoAmount)
				break;
			}
		...
		console.log("          ++ Fiat:", fiatBuys, sell.fiatAmount)
	}
}
